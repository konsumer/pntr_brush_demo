    ...
    for (i = 0; i < brush->pathSize; i++) {
        ...
        switch (action->type) {
            ...
            case PNTR_BRUSH_ACTION_TYPE_ARC: {
                pntr_draw_arc_thick(brush->dst, action->position.x, action->position.y, action->radius, radToDeg(action->startAngle), radToDeg(action->endAngle), action->segments, brush->lineWidth, brush->strokeStyle);
            }
            break;
        }
    }
